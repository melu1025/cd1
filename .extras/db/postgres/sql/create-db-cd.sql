...
--        chown postgres:postgres /var/lib/postgresql/tablespace/cd
...
-- (5) 2. PowerShell:
--     docker compose exec db bash
--        psql --dbname=postgres --username=postgres --file=/sql/create-db-cd.sql
--        psql --dbname=cd --username=cd --file=/sql/create-schema-cd.sql
--        exit
--      docker compose down
...
CREATE ROLE cd LOGIN PASSWORD 'p';

-- https://www.postgresql.org/docs/current/sql-createdatabase.html
CREATE DATABASE cd;

GRANT ALL ON DATABASE cd TO cd;

-- https://www.postgresql.org/docs/10/sql-createtablespace.html
CREATE TABLESPACE cdspace OWNER cd LOCATION '/var/lib/postgresql/tablespace/cd';
